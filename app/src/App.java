...
class App extends JFrame implements ActionListener {
  ...
  App(String user, String pass) {
    ...
    heading.setBounds(180,200,100,100);
    tab1.add(id);
    id.setFont(font1);
    tab1.add(t4);
    tab1.add(name);
    ...
    tableModel =
      new DefaultTableModel(
        new Object[] { "Employee ID", "Name", "Salary", "Age" },
        0
      );
  ...
  public void insert() {
    try {
      ...
      try {
        st.execute(
          "CREATE TABLE Employee (id INT PRIMARY KEY, name VARCHAR(30) , salary INT, age INT ) "
        );
      } catch (Exception eq) {
        while (i >= 0) {
